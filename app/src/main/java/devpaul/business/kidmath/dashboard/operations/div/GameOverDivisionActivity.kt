...
class GameOverDivisionActivity : AppCompatActivity() {

    var TAG = "GameOverDivision"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_game_over_div)
        ...
        sharedPreferences = getSharedPreferences("prefdiv", 0)
    ...
    private fun sendData(){
        ...
        FirebaseAuth.getInstance().currentUser?.metadata?.apply {
            ...
            db.collection(Constants.PATH_POINTS_DIV).document(uiduser.toString())
                .set(dataPoints)
                .addOnSuccessListener {
                    Log.v(TAG,"Success : $it")
                    db.collection("AllResultsDiv").document(randomUUID).set(dataPoints);
                    myref.child("AllResultsDiv").child(randomUUID).setValue(dataPoints)
                }
    ...
    fun exit(view: View?) {
        val intent = Intent(this@GameOverDivisionActivity, HomeActivity::class.java)
    ...
    private fun nextlevel() {
        val level = intent.getStringExtra("level")
        if (level == "Facil") {
            val intent = Intent(this@GameOverDivisionActivity, DivGameActivity::class.java)
