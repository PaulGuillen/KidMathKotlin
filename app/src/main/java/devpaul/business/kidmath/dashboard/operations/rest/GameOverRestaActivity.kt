package devpaul.business.kidmath.dashboard.operations.rest
...
class GameOverRestaActivity : AppCompatActivity() {
        ...
        FirebaseAuth.getInstance().currentUser?.metadata?.apply {
            ...
            db.collection(Constants.PATH_POINTS_RES).document(uiduser.toString())
                ...
                .addOnSuccessListener {
                    ...
                    db.collection("AllResultsRes").document(randomUUID).set(dataPoints);
                    myref.child("AllResultsRes").child(randomUUID).setValue(dataPoints)
                }
    ...
    private fun nextlevel() {
        val level = intent.getStringExtra("level")
        if (level == "Facil") {
            val intent = Intent(this@GameOverRestaActivity, RestGameActivity::class.java)
